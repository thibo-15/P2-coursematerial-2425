...
def count_movies_from_year(movies, year_of_publication):
    count = 0
    for movie in movies:
        if movie.year == year_of_publication:
            count += 1
    return count
...
selected_movies = select_movies_with_actor(movies, actor_name)

print(f"Films met {actor_name}:")
for movie in selected_movies:
    print("-", movie.title)"""

def sum_movies_duration_from_period(movies, period):
    total_duration = 0
    for movie in movies:
        if period[0] <= movie.year <= period[1]: 
            total_duration += movie.runtime  
    return total_duration

"""period = (2000, 2020)
total_runtime = sum_movies_duration_from_period(movies, period)
print({total_runtime})"""

def get_directors_for_actors(movie, actor):
    directors = []
    for movie in movies:
        if actor in movie.actors:
...
print(get_directors_for_actors(movies, actor))"""
...
"""letter = "k"
strings = ["Ksnap", "eindelijk", "iets"]  # Nu een lijst van woorden
print(find_string_starting_with(strings, letter))"""

def find_number_greater_than(numbers, greater_than ):
...
"""numbers = [1, 3, 5, 7, 9]
result = find_number_greater_than(numbers, 4)
print(result)"""
